...
class Boilerplate(object):
    ...
    def gen_comments(self):
        """ Generate the docstring for the first few lines of the program. """
          # line return
        header_comments_section = '"""' + self.lr
        header_comments_section += self.args.name + self.lr
        header_comments_section += self.args.user + self.lr
        header_comments_section += self.args.date + self.lr
        header_comments_section += self.args.desc + self.lr
        header_comments_section += '"""' + self.lr
    ...
    def gen_imports(self):
        ...
        #[self.write_to_file(name + self.lr) for name in imports]
        for name in self.imports:
            self.write_to_file("import " + name + self.lr)
    ...
    def gen_sections(self):
        ...
        def gen_section_text(name, params):
            ...
            print(name, params)
            section_text = ''
            section_text += self.lr * 2
            section_text += 'def '
            section_text += name + '('+ params + '):' + self.lr
            section_text += ' ' * 4
            section_text += '""" Doc string for ' + name +'. """' + self.lr
            section_text += ' ' * 4
            section_text += 'pass'
            section_text += self.lr
            section_text += ' ' * 4
    ...
    def gen_tests(self):
        ...
        def gen_assert_tests(function_name, test_name):
            ...
            test_text += self.lr
            test_text += ' ' * 4
            test_text += '""" Unittest """' + self.lr
            test_text += ' ' * 4
            test_text += '# pylint: disable=R0904' + self.lr
            test_text += ' ' * 4
            test_text += 'def setUp(self):' + self.lr
            test_text += ' ' * 8
            test_text += 'self.results = (None)' + self.lr
            test_text += ' ' * 8
            test_text += 'self.test_data = (123)  # change this!!!!'
            test_text += self.lr * 2
            test_text += ' ' * 4
            test_text += 'def test_function(self):' + self.lr
            test_text += ' ' * 8
            test_text += '""" Test function Assert Equal """' + self.lr
            test_text += ' ' * 8
            test_text += test_name + '(self.results, ' + function_name
            test_text += '(self.test_data))' + self.lr
